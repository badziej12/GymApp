...
export const DayCarousel:FC<DayCarouselProps> = ({ daysRow, onDateSelect, showFullMonth }) => {
    ...
    const handleDateSelect = (day: Date, index: number) => {
    ...
    return (
        <View className="flex-row" style={{display: showFullMonth ? "none" : "flex", height: itemWidth, gap: itemGap, transform: `translateX(${-translateState}px)`, transitionDuration: isTranslating ? '' : '.4s'}}>
            {daysRow.map((day, index) => (
                <Pressable onPress={() => isTranslating ? handleDateSelect(day, index) : null} key={index} style={styles.dayContainer} className={"flex flex-col items-center"}>
